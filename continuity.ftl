...
a unequally converges to x iff
for any eps there exists a positive integer N
such that for any n if N<n then 0<d(a(n),x)<eps.

Definition LimitPoint. Let E be a subset of Real.
A limit point of E is a real number p such that
there exists a sequence a such that a is into E
...
Axiom d3. Let z be a real number.
d(x,z) <= d(x,y) + d(y,z).
Axiom d4. d(x,y) >= 0.
Axiom d5. If x > 0 then d(x,0) = x.

Axiom IsolatedPoint. Let E be a subset of Real.
Suppose p is not a limit point of E.
Then there exists eps such that for all x
if 0<d(x,p)<eps then x is not in E.
...
If a converges to x and b converges to y
then a++b converges to x+y
and a**b converges to x*y.
...
if a unequally converges to p
then f \circ a converges to q.
Proof.
Let us show that if lim(f,p)=q
then for any sequence a into Dom(f)
if a unequally converges to p
then f \circ a converges to q.
  Suppose lim(f,p)=q.
  ...
  Let us show that f \circ a converges to q.
    ...
    Take del such that for any x \in Dom(f)
    if 0<d(x,p)<del then d(f(x),q)<eps.
    Take a positive integer N such that
    for any n if N<n then 0<d(a(n),p)<del.
    For any n if N<n then d(f(a(n)),q)<eps.
  End.
End.
Let us show that if (for any sequence a if
a is into Dom(f) and a unequally converges to p
then f \circ a converges to q)
then lim(f,p)=q.
  Suppose for any sequence a if
  a is into Dom(f) and a unequally converges to p
  then f \circ a converges to q.
  Let us show that lim(f,p)=q.
    Suppose the contrary.
    Take eps such that for any del
    there exists x \in Dom(f) such that
    0<d(x,p)<del and not d(f(x),q)<eps.
    Take a sequence b such that b converges to 0
    and for all n b(n)>0 (by 3_1).
    For all n d(b(n),0)=b(n) (by d5).
    Define a(n) = Choose x \in Dom(f)
    such that 0<d(x,p)<d(b(n),0) and
    not d(f(x),q)<eps in x for n in Natural.
    Let us show that a unequally converges to p.
      Suppose r is a positive real number.
      Take a positive integer N such that
      for any n if N<n then d(b(n),0)<r.
...
Then q1 = q2.
Proof.
Take a sequence a such that a is into Dom(f)
and a unequally converges to p.
...
Theorem 4_4. Suppose Dom(f) = Dom(g).
Suppose p is a limit point of Dom(f).
...
S = Dom(fsg) = Dom(fmg).
Let us show that for any sequence a into S
if a unequally converges to p then
fsg \circ a converges to A+B and
fmg \circ a converges to A*B.
  Suppose a is a sequence such that
  a is into S and a unequally converges to p.
...
Suppose p is not a limit point of Dom(f).
Then f is continuous at p.
Proof.
Take del such that for all x
if 0<d(x,p)<del then x is not in Dom(f).
End.
...
Theorem 4_7. Suppose p \in Dom(f).
...
Take a positive real number eta
such that for any x \in Dom(f)
if d(x,p)<eta then d(f(x),f(p))<del.
QED.
...
Theorem 4_9. Suppose Dom(f) = Dom(g).
Suppose p \in Dom(f).
...
Case p is a limit point of Dom(f).
  ...
  f++g is continuous at p and
  f**g is continuous at p (by 4_6).
End.
Case p is not a limit point of Dom(f).
  f++g is continuous at p and
  f**g is continuous at p (by 4_5a).
End.
QED.